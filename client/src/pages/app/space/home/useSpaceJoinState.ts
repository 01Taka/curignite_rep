...
export const useSpaceJoinState = (uid: string | null, currentSpaceId: string | null, currentSpace: SpaceData | undefined) => {
  ...
  useEffect(() => {
    const updateJoinState = async () => {
      if (uid && currentSpaceId) {
        ...
        const state = await spaceService.getSpaceJoinState(uid, currentSpaceId, currentSpace);
        setJoinState(state);
      }
    };
    updateJoinState();
